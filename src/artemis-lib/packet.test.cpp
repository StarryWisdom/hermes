...
TEST_F (artemis_packet_test,object_bitstream) {
	...
	buffer=artemis_packet::server_to_client::make_object_bitstreams(tmp2).at(0);
	EXPECT_EQ(buffer.buffer.size(),29u);
	check_artemis_header(artemis_packet::direction::server_to_client);
	EXPECT_EQ(buffer.read<uint32_t>(),artemis_packet::server_to_client::object_bit_stream_jam32);
...
TEST_F (artemis_packet_test,no_data) {
	bool ret=artemis_packet::fetch_artemis_packet(buffer,[](uint32_t){});
	EXPECT_EQ(ret,false);
}

TEST_F (artemis_packet_test,only_header) {
	packet_buffer packet;
	bool first{true};
	bool ret=artemis_packet::fetch_artemis_packet(packet,
		[&packet, &first] (uint32_t len) {
			if (first) {
				EXPECT_EQ(len,20u);
				packet.write<uint32_t>(0xdeadbeef);
				packet.write<uint32_t>(20);
				packet.write<uint32_t>(2);
				packet.write<uint32_t>(0);
				packet.write<uint32_t>(0);
				EXPECT_EQ(packet.write_offset,20u);
			}
			first=false;
		});
	EXPECT_EQ(ret,true);
...
TEST_F (artemis_packet_test,split_only_header) {
	packet_buffer packet;
	bool first{true};
	bool ret=artemis_packet::fetch_artemis_packet(packet,
		[&packet, &first] (uint32_t len) {
			if (first) {
				...
				packet.write<uint32_t>(2);
				packet.write<uint32_t>(0);
				EXPECT_EQ(packet.write_offset,16u);
			}
			first=false;
		});
	first=true;
	EXPECT_EQ(ret,false);
	ret=artemis_packet::fetch_artemis_packet(packet,
		[&packet, &first] (uint32_t len) {
			if (first) {
				EXPECT_EQ(len,4u);
				packet.write<uint32_t>(0);
...
TEST_F (artemis_packet_test,full_packet) {
	packet_buffer packet;
	uint8_t i=0;
	bool ret=artemis_packet::fetch_artemis_packet(packet,
		[&packet, &i] (uint32_t len) {
			if (i==0) {
				packet.write<uint32_t>(0xdeadbeef);
				packet.write<uint32_t>(24);
				packet.write<uint32_t>(2);
				packet.write<uint32_t>(0);
				packet.write<uint32_t>(4);
				EXPECT_EQ(len,20u);
				EXPECT_EQ(packet.write_offset,20u);
			} else if (i==1) {
				packet.write<uint32_t>(4);
				EXPECT_EQ(len,4u);
				EXPECT_EQ(packet.write_offset,24u);
			}
			i++;
		});
	EXPECT_EQ(ret,true);
...
TEST_F (artemis_packet_test,split_full_packet) {
	packet_buffer packet;
	uint8_t i=0;
	bool ret=artemis_packet::fetch_artemis_packet(packet,
		[&packet, &i] (uint32_t len) {
			if (i==0) {
				...
				packet.write<uint8_t>(4);
				EXPECT_EQ(len,2u);
				EXPECT_EQ(packet.write_offset,21u);
			}
			i++;
		});
	EXPECT_EQ(ret,false);
	...
	ret=artemis_packet::fetch_artemis_packet(packet,
		[&packet, &first] (uint32_t len) {
			if (first) {
				...
				EXPECT_EQ(len,1u);
				EXPECT_EQ(packet.write_offset,22u);
			}
			first=false;
		});
	EXPECT_EQ(ret,true);
